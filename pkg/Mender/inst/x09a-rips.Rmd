---
title: "Cytangle: Topological Data Analysis"
...
for (N in rownames(cellCountsA)) {   # nodes
  for (S in colnames(cellCountsA)) { # samples
    ...
    if (is.na(pcdimA[N, S]) | pcdimA[N, S] < 3) next
    count <- count + 1
    ...
    rips <- ripsDiag(keeper, 2, 5, library="Dionysus", location=TRUE)
...
for (N in rownames(cellCountsB)) {   # nodes
  for (S in colnames(cellCountsB)) { # samples
    ...
    if (is.na(pcdimB[N, S]) | pcdimB[N, S] < 3) next
...
```{r AB}
f <- file.path(paths$scratch, "collate.Rda")
if (file.exists(f)) {
  load(f)
} else {
