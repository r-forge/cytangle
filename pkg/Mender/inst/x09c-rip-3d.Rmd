---
title: "Cytangle: Topological Data Analysis (3D)"
...
```{r directories}
ripDir <- file.path(paths$scratch, "rip3d")
...
for (N in rownames(cellCountsA)) {   # nodes
  for (S in colnames(cellCountsA)) { # samples
    ...
    if (is.na(pcdimA[N, S]) | pcdimA[N, S] < 4) next # only do case of pcd >= 4
    count <- count + 1
    ...
    cat("Working on", S, "at node", N,
        "with", nrow(M), "cells.\n", file=stderr()) 
    spca <- SamplePCA(t(M))
    usable <- spca@scores[, 1:4]
    if (nrow(usable) > 120) {
      keeper <- downSample(usable)
    } else {
      keeper <- usable
    }
    ## look up to 3D topological holes (1 = loops, 2 = spheres, 3 = hyper...)
    rips <- ripsDiag(keeper, 3, 5, library="Dionysus", location=TRUE)
...
for (N in rownames(cellCountsB)) {   # nodes
  for (S in colnames(cellCountsB)) { # samples
    ...
    if (is.na(pcdimB[N, S]) | pcdimB[N, S] < 4) next
...
```{r AB}
f <- file.path(paths$scratch, "collate3d.Rda")
if (file.exists(f)) {
  load(f)
} else {
