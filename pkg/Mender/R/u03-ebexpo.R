...
gammaFit <- function(edata, resn = 200) {
  ...
  shape <- mu^2/sigma^2
  rate <- mu/sigma^2
  h0 <- hist(d0, breaks = 123, prob = TRUE, plot = FALSE)
  X0 <- h0$mids
  pdf <- h0$density
...
expoFit  <- function(edata, resn = 200) {
  ...
  h0 <- hist(edata, breaks=bks, plot = FALSE, freq = FALSE)
  X0 <- h0$mids
  pdf <- h0$density
...
plotExpoFit <- function(object) {
  delta <- getDelta(recip <- 1/object$mu)
  ss <- seq(delta[1], delta[2], length=300)
  tt <- sapply(ss, expoSSE, X0 = object$X0, pdf = object$pdf)
  plot(ss, tt, type='l', xlab="Lambda", ylab="SS_Error", lwd=2)
  points(object$lambda, expoSSE(object$lambda, object$X0, object$pdf),
         pch=16, col="red", cex=1.5)
  points(recip, expoSSE(1/object$mu, object$X0, object$pdf),
         pch=16, col="blue", cex=1.5)
  legend("top", c("lambda", "1/mu"), col=c("red", "blue"), pch=16)
  invisible(object)
}

EBexpo <- function(object) {
  ## fit the model
  X0 <- object$X0
  ...
  val  <- list(expo = object,
               theoretical.pdf = theo,
               unravel = unravel)
  val
}

histEBexpo <- function(x, xlab="", ylab="Prob(Interesting | X)", main="", ...) {
...
plotEB <- function(x, prior, post = c(0.5, 0.8, 0.9)) {
