...
featAge <- Feature(clinical[,"AgeAtDx"], "Age at Diagnosis", c("white", "purple"), c("High", "Low"))
opar <- par(mfrow = c(1,2))
...
```{r recycle}
poof <- Mender:::angleMean(W, ripdiag, cyc1@index, clinical)
poof[is.na(poof)] <- 0
angle.df <- poof[, c("AgeAtDx", "Sex", "mutation.status", "Light.chain.subtype", "CatRAI", "CatB2M")]
col.ls <- c("red", "green", "blue", "orange", "purple", "brown")
...
```{r}
angle.df <- poof[, c("LogB2M", "LogLDH", "LogWBC", "Hemoglobin", "Platelets", "Prolymphocytes")]
opar <- par(cex = 1.5, mai = c(0, 0, 0, 0))
circos.clear()
circos.par(track.height = 0.08, 
           start.degree = 90)
...
```{r}
angle.df <- poof[, c("CatWBC", "CatCD38", "Hypogammaglobulinemia", "Massive.Splenomegaly", "Matutes", "Prolymphocytes")]
opar <- par(cex = 1.5, mai = c(0, 0, 0, 0))
circos.clear()
circos.par(track.height = 0.08, 
           start.degree = 90)
...
```{r}
angle.df <- poof[, c("stat17", "stat12", "stat13", "stat11", "mutation.status", "CatRAI")]
opar <- par(cex = 1.5, mai = c(0, 0, 0, 0))
circos.clear()
circos.par(track.height = 0.08, 
           start.degree = 90)
...
```{r}
angle.df <- poof[, c("mutation.status", "CatCD38", "CatB2M", "CatRAI", "Massive.Splenomegaly", "Hypogammaglobulinemia")]
col.lo <- c("green",  "cyan", "blue",   "#dddddd",   "#dddddd", "#dddddd")
col.hi <- c("magenta", "red" , "yellow", "black", "brown", "purple")
opar <- par(cex = 1.5, mai = c(0, 0, 0, 0))
circos.clear()
circos.par(track.height = 0.08, 
           start.degree = 90)
...
for(i in 1:length(angle.df[1,])) {
  ...
  if(i == 1) {
    data <- as.data.frame(angle.df[,i])
    col <- colorRamp2(c(min(data), max(data)), c(col.lo[i], col.hi[i]))
    circos.heatmap(data, rownames.side = "outside", 
                   cluster = FALSE, col = col, rownames.cex = 1)
  } else {
    data <- as.data.frame(angle.df[,i])
